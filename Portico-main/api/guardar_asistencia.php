...
function guardarMaquinaria(PDO $pdo, string $fecha, int $id_obra): void
{
    ...
    // Mantener la relación obra-maquinaria
    ...
    // Registrar la asistencia de la maquinaria
    ...
    foreach ($_POST['maquinaria'] as $id_maquinaria) {
        ...
        $horaSalida = $_POST['hora_salida_maquinaria'][$id_maquinaria] ?? '';
        $horaDevolucion = $_POST['hora_devolucion_maquinaria'][$id_maquinaria] ?? '';

        if (
            empty($horaSalida) ||
            empty($horaDevolucion) ||
            !preg_match('/^\d{2}:\d{2}$/', $horaSalida) ||
            !preg_match('/^\d{2}:\d{2}$/', $horaDevolucion)
        ) {
            throw new InvalidArgumentException(
                "Debe ingresar la hora de salida y devolución de la maquinaria."
            );
        }

        // Guarda la maquinaria asignada a la obra
        $stmtObra->execute([
            $id_obra,
            $id_maquinaria,
            $fecha
