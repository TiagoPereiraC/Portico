...
maquinariaForm.addEventListener("submit", async (event) => {
  ...
  const eraEdicion = itemEnEdicion !== null;

  btnGuardar.disabled = true;
  btnGuardar.textContent = eraEdicion ? "Actualizando..." : "Guardando...";
  ...
  try {
    ...
    // mantener la página actual si era edición
    if (!eraEdicion) {
      paginaActual = 1;
    }

    // forzar recarga completa del listado
