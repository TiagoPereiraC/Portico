...
function renderMaquinaria() {
  ...
  maquinaria.forEach((item) => {
    ...
    if (
      ...
      item.vencimiento !== "" &&
      !String(item.vencimiento).startsWith("0000")
    ) {
      vencimiento = formatDate(item.vencimiento);

      const partes = String(item.vencimiento)
        .substring(0, 10)
        .split("-");
      ...
      if (diffDays < 0) {
        ...
        certIcono =
          '<i class="fas fa-circle-exclamation" style="color:#b91c1c; margin-left:6px;" title="Certificado vencido"></i>';

      } else if (diffDays <= 30) {
        row.classList.add("table-warning");

        certIcono =
          `<i class="fas fa-triangle-exclamation" style="color:#856404; margin-left:6px;" title="Vence en ${diffDays} día${diffDays === 1 ? "" : "s"}"></i>`;
      }
    }
      ...
      <td>
        <div class="table-actions">
          ...
          <button
            ...
            class="action-btn"
            data-action="cert"
            data-id="${item.id_maquinaria}"
            title="Certificados"
            style="background:#e8e3df;color:#666;"
          >
  ...
  paginationInfo.textContent =
    `Mostrando ${desde}-${hasta} de ${totalItems} ${totalItems === 1 ? "equipo" : "equipos"}`;
