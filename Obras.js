...
const detailCombustibleWrap = document.getElementById("detailCombustibleWrap");
const detailCombustibleBody = document.getElementById("detailCombustibleBody");
const detailCombustibleEmpty = document.getElementById("detailCombustibleEmpty");
const detailCombustibleTotalLitros = document.getElementById("detailCombustibleTotalLitros");
const detailCombustibleTotalCosto = document.getElementById("detailCombustibleTotalCosto");
...
const detailTareasCompletadasWrap = document.getElementById("detailTareasCompletadasWrap");
const detailTareasCompletadasBody = document.getElementById("detailTareasCompletadasBody");
const detailTareasCompletadasEmpty = document.getElementById("detailTareasCompletadasEmpty");
const detailTareasPendientesWrap = document.getElementById("detailTareasPendientesWrap");
const detailTareasPendientesBody = document.getElementById("detailTareasPendientesBody");
const detailTareasPendientesEmpty = document.getElementById("detailTareasPendientesEmpty");
...
const API_URL = apiBase ? `${apiBase}/Obras.php` : null;
...
document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && obraDetailModal && !obraDetailModal.classList.contains("hidden")) {
        cerrarDetalle();
    }
});
...
document.addEventListener("click", async (event) => {
    ...
    if (button.dataset.action === "edit") {
        ...
        const obra = obras.find((item) => Number(item.id_obra) === idObra);
        if (!obra) return;
        ...
        abrirConfirmacion({
            ...
            onAccept: async () => {
                try {
                    ...
                    fillForm({ ...data.obra, tareas: data.tareas || [] });
                    setFeedback(`Editando ${obra.nombre}.`, "info");
                } catch (error) {
                    console.error(error);
                    setFeedback(error.message || "No se pudieron cargar las actividades de la obra.", "error");
                }
            }
        });
        return;
        ...
        abrirConfirmacion({
            ...
            onAccept: async () => {
                try {
                    ...
                    if (Number(obraEnEdicion) === idObra) {
                        resetForm();
                    }
...
async function inicializarVista() {
    ...
    if (!apiBase && !isDesktopWebView) {
        setLoading(false);
        throw new Error("Abrí esta pantalla desde navegador con PHP o desde la aplicación de escritorio.");
    }
...
async function buildPayload() {
    ...
    const payload = {
        ...
        fecha_fin: obraEnEdicion
            ? (obras.find((item) => Number(item.id_obra) === Number(obraEnEdicion))?.fecha_fin || "")
            : "",
        nombre_cliente: leerCampo("nombre_cliente"),
        telefono_cliente: leerCampo("telefono_cliente"),
        activo: activoElement?.checked ? 1 : 0,
        tareas: tareasContrato.map((tarea) => ({
            id_tarea: tarea.id_tarea || null,
    ...
    for (const tarea of tareasContrato) {
        if (!String(tarea.descripcion || "").trim()) {
            setFeedback("Todas las actividades deben tener una descripción.", "error");
            return null;
        }
...
async function guardarObra(payload) {
    if (apiBase) {
        ...
        if (!response.ok) throw new Error(data.error || "No se pudo guardar la obra.");
        return data;
    }
...
async function eliminarObra(idObra) {
    if (apiBase) {
        ...
        if (!response.ok) throw new Error(data.error || "No se pudo eliminar la obra.");
        return data;
    }
...
async function descargarContrato(idObra) {
    if (apiBase) {
        const response = await fetch(
            `${API_URL}?id_obra=${encodeURIComponent(idObra)}&descargar_contrato=1`,
            { credentials: "include" }
        );
        ...
        const nombreArchivo =
            ...
            obras.find((item) => Number(item.id_obra) === Number(idObra))?.contrato_nombre_archivo ||
            `contrato-${idObra}`;
...
async function cargarObras() {
    ...
    try {
        ...
        if (apiBase) {
            ...
            data = await fetchJson(`${API_URL}?${params.toString()}`);
        } else {
        ...
        if (solicitudId !== ultimaSolicitudListado) return;
...
function renderObras() {
    ...
    emptyState.classList.toggle("hidden", hayObras);
    pagination.classList.toggle("hidden", !hayObras || totalPaginas === 1);

    if (!hayObras) {
        emptyStateMessage.textContent = obtenerMensajeSinResultados(terminoBusqueda.trim(), filtroEstado);
    ...
    const desde = (paginaActual - 1) * OBRAS_POR_PAGINA + 1;
...
function fillForm(obra) {
    ...
    if (activo) {
        activo.checked = obra.activo === 1 || obra.activo === "1";
    }
    ...
    tareasContrato = Array.isArray(obra.tareas)
        ? obra.tareas.map((tarea) => ({
            id_tarea: tarea.id_tarea ? Number(tarea.id_tarea) : null,
            id_tarea_cliente: generarIdTareaCliente(),
            id_tarea_origen: tarea.id_tarea_origen ? Number(tarea.id_tarea_origen) : null,
...
async function leerContratoSeleccionado() {
    ...
    try {
        ...
        if (commaIndex === -1) throw new Error("Formato de archivo inválido.");
...
function actualizarEstadoContratoSeleccionado() {
    ...
    const obraActual = obras.find((item) => Number(item.id_obra) === Number(obraEnEdicion));
...
function setFeedback(message, type = "info") {
    ...
    if (feedbackTitle) {
        feedbackTitle.textContent =
            type === "error" ? "No se pudo completar la acción" :
...
function cerrarConfirmacion() {
    ...
    const btnFinalizar = document.getElementById("confirmFinalizeContract");
...
async function cargarDetalleObra(idObra) {
    ...
    if (apiBase) {
        data = await fetchJson(`${API_URL}?id_obra=${encodeURIComponent(idObra)}&detalle=1`);
    } else {
        data = await sendDesktopRequest(
            "obras_detalle",
            { id_obra: idObra },
...
function renderDetalle(data) {
    ...
    detailStatus.className = "detail-status" + (esActiva ? "" : " inactive");
        ...
        detailMaterialesBody.innerHTML = materiales.map((m) => {
            ...
            return `
                <tr>
                    ...
                    <td>${formatCurrency(costoTotal)}</td>
                </tr>
            `;
        }).join("");

        if (detailMaterialesTotal) {
            detailMaterialesTotal.textContent = formatCurrency(granTotalMateriales);
        }
    } else {
        detailMaterialesWrap.classList.add("hidden");
        detailMaterialesEmpty.classList.remove("hidden");
        detailMaterialesBody.innerHTML = "";
        if (detailMaterialesTotal) detailMaterialesTotal.textContent = formatCurrency(0);
    }
    ...
    if (obreros.length) {
        ...
        detailObrerosBody.innerHTML = obreros.map((o) => {
            const nombreCompleto = `${o.apellido || ""}, ${o.nombre || ""}`.trim();
            const horas = Number(o.horas_totales || 0);

            return `
                <tr>
                    <td>${escapeHtml(nombreCompleto || "—")}</td>
                    <td>${horas.toLocaleString("es-UY", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
            `;
        }).join("");
    } else {
    ...
    if (combustible.length) {
        if (detailCombustibleWrap) detailCombustibleWrap.classList.remove("hidden");
        if (detailCombustibleEmpty) detailCombustibleEmpty.classList.add("hidden");

        let granTotalLitros = 0;
        let granTotalCosto = 0;

        if (detailCombustibleBody) {
            detailCombustibleBody.innerHTML = combustible.map((c) => {
                ...
                const maqNombre = c.maquinaria_nombre || "Sin asignar";
                const maqMarca = c.maquinaria_marca ? ` (${c.maquinaria_marca})` : "";
                const maqTexto = maqNombre !== "Sin asignar" ? `${maqNombre}${maqMarca}` : "General / Sin asignar";

                return `
                    <tr>
                        <td>${formatDate(c.fecha)}</td>
                        <td>${escapeHtml(maqTexto)}</td>
                        <td><span class="badge-combustible">${escapeHtml(c.nombre_combustible || "Diesel")}</span></td>
                        <td>${litros.toLocaleString("es-UY", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} L</td>
                        <td>${formatCurrency(precioUnitario)}</td>
                        <td><strong>${formatCurrency(precioTotal)}</strong></td>
                    </tr>
                `;
            }).join("");
        }
        ...
        if (detailCombustibleTotalLitros) {
            detailCombustibleTotalLitros.textContent = `${granTotalLitros.toLocaleString("es-UY", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} L`;
        }

        if (detailCombustibleTotalCosto) {
            detailCombustibleTotalCosto.textContent = formatCurrency(granTotalCosto);
        }
    } else {
        if (detailCombustibleWrap) detailCombustibleWrap.classList.add("hidden");
        if (detailCombustibleEmpty) detailCombustibleEmpty.classList.remove("hidden");
        if (detailCombustibleBody) detailCombustibleBody.innerHTML = "";
        if (detailCombustibleTotalLitros) detailCombustibleTotalLitros.textContent = "0,00 L";
        if (detailCombustibleTotalCosto) detailCombustibleTotalCosto.textContent = formatCurrency(0);
    }
    ...
    const tareasCompletadas = tareas.filter((tarea) => String(tarea.estado || "").toLowerCase() === "completada");
    const tareasPendientes = tareas.filter((tarea) => String(tarea.estado || "").toLowerCase() !== "completada");
    const totalCompletadas = tareasCompletadas.length;
    const totalPendientes = tareasPendientes.length;
    const ganadoHastaAhora = tareasCompletadas.reduce((total, tarea) => total + Number(tarea.importe || 0), 0);

    if (detailTareasTotal) detailTareasTotal.textContent = totalTareas;
    if (detailTareasCompletadas) detailTareasCompletadas.textContent = totalCompletadas;
    if (detailTareasPendientes) detailTareasPendientes.textContent = totalPendientes;
    if (detailTareasGanado) detailTareasGanado.textContent = formatCurrency(ganadoHastaAhora);

    // ACTIVIDADES COMPLETADAS
    if (detailTareasCompletadasWrap && detailTareasCompletadasBody && detailTareasCompletadasEmpty) {
        if (tareasCompletadas.length) {
            ...
            detailTareasCompletadasBody.innerHTML = tareasCompletadas.map((tarea) => `
                <tr>
                    <td>${escapeHtml(tarea.descripcion || "—")}</td>
                    <td>${formatCurrency(Number(tarea.importe || 0))}</td>
                    <td>${tarea.fecha_completada ? formatDate(tarea.fecha_completada) : "—"}</td>
                </tr>
            `).join("");
        } else {
    ...
    if (detailTareasPendientesWrap && detailTareasPendientesBody && detailTareasPendientesEmpty) {
        if (tareasPendientes.length) {
            ...
            detailTareasPendientesBody.innerHTML = tareasPendientes.map((tarea) => `
                <tr>
                    <td>${escapeHtml(tarea.descripcion || "—")}</td>
                    <td>${formatCurrency(Number(tarea.importe || 0))}</td>
                </tr>
    ...
    if (detailBtnEdit) detailBtnEdit.disabled = false;
    if (detailBtnDownload) detailBtnDownload.disabled = !obra.contrato_nombre_archivo;
    if (detailBtnDelete) detailBtnDelete.disabled = false;
    if (detailBtnToggleStatus) detailBtnToggleStatus.disabled = false;
    if (detailBtnMap) detailBtnMap.disabled = !obra.direccion;

    // MAPA
    if (detailBtnMap && detailMapSection && detailMapIframe) {
        detailBtnMap.onclick = async (event) => {
            ...
            try {
                ...
                if (geo.length) {
                    ...
                    const bbox = [lon - margenLon, lat - margenLat, lon + margenLon, lat + margenLat].join(",");

                    detailMapIframe.src = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${lat},${lon}`;
                } else {
    ...
    // ESTADO / CONTRATO
    if (detailToggleStatusText && detailBtnToggleStatus) {
        if (esActiva) {
            detailToggleStatusText.textContent = "Cerrar contrato";
            detailBtnToggleStatus.className = "btn-save";
        } else {
            detailToggleStatusText.textContent = "Activar Obra";
            detailBtnToggleStatus.className = "btn-secondary";
        }
    }
    ...
    if (detailBtnToggleStatus) {
        detailBtnToggleStatus.onclick = (event) => {
            ...
            if (!esActiva) {
                abrirConfirmacion({
                    title: "Activar obra",
                    message: `¿Querés volver a activar la obra "${obra.nombre}"?`,
                    acceptLabel: "Activar",
                    onAccept: async () => {
                        try {
                            if (apiBase && !csrfToken) csrfToken = await obtenerCsrf();
                            await cambiarEstadoObra(obra.id_obra, 1);
                            await cargarObras();
                            setFeedback(`La obra "${obra.nombre}" fue activada correctamente.`, "success");
                        } catch (error) {
                            console.error(error);
                            setFeedback(error.message || "No se pudo activar la obra.", "error");
                        }
                    }
                });
                return;
            ...
            abrirConfirmacion({
                ...
                message: `¿Qué querés hacer con la obra "${obra.nombre}"?\n\nNuevo contrato: cerrar el contrato actual y crear uno nuevo.\nFinalizar obra: cerrar el contrato y finalizar definitivamente la obra.`,
                acceptLabel: "Nuevo contrato",
                onAccept: async () => {
                    abrirModalNuevoContrato(obra);
                }
            });

            prepararOpcionFinalizarObra(obra);
        };
    }
    ...
    if (detailBtnDownload) {
        detailBtnDownload.onclick = async (event) => {
            ...
            if (!obra.contrato_nombre_archivo) {
                setFeedback("Esta obra no tiene un contrato cargado.", "info");
                return;
            }
            ...
            abrirConfirmacion({
                ...
                onAccept: async () => {
                    try {
                        ...
                        if (Number(obraEnEdicion) === Number(obra.id_obra)) {
                            resetForm();
                        }
    ...
    if (detailBtnEdit) {
        detailBtnEdit.onclick = (event) => {
            ...
            abrirConfirmacion({
                ...
                onAccept: async () => {
                    try {
                        const data = await obtenerDetalleObra(Number(obra.id_obra));
                        fillForm({ ...data.obra, tareas: data.tareas || [] });
                        setFeedback(`Editando ${obra.nombre}.`, "info");
                    } catch (error) {
                        console.error(error);
                        setFeedback(error.message || "No se pudieron cargar las actividades.", "error");
                    }
                }
            });
        };
...
function prepararOpcionFinalizarObra(obra) {
    if (!confirmModal || !confirmAccept) return;

    const existente = confirmModal.querySelector("#confirmFinalizeContract");
    if (existente) existente.remove();

    const btnFinalizar = document.createElement("button");
    btnFinalizar.type = "button";
    btnFinalizar.id = "confirmFinalizeContract";
    btnFinalizar.className = "btn-secondary";
    btnFinalizar.textContent = "Finalizar obra";
    ...
    btnFinalizar.addEventListener("click", () => {
        ...
        abrirConfirmacion({
            ...
            message: `¿Estás seguro de que querés finalizar la obra "${obra.nombre}"?\n\nEl contrato actual se cerrará y la obra quedará inactiva.`,
            acceptLabel: "Finalizar obra",
            onAccept: async () => {
                try {
                    if (apiBase && !csrfToken) csrfToken = await obtenerCsrf();
                    await cerrarContrato(obra.id_obra, "finalizar_obra");
                    await cargarObras();
                    setFeedback(`La obra "${obra.nombre}" fue finalizada correctamente.`, "success");
                } catch (error) {
                    console.error(error);
                    setFeedback(error.message || "No se pudo finalizar la obra.", "error");
                }
            }
        });
    });

    const botones = confirmAccept.parentElement;
    if (botones) {
        botones.insertBefore(btnFinalizar, confirmAccept);
    }
}
...
async function cambiarEstadoObra(idObra, activo) {
    if (apiBase) {
        ...
        if (!response.ok) throw new Error(data.error || "No se pudo cambiar el estado de la obra.");
        return data;
    }
...
async function obtenerDetalleObra(idObra) {
    if (apiBase) {
        return await fetchJson(`${API_URL}?id_obra=${encodeURIComponent(idObra)}&detalle=1`);
    }
...
function agregarTarea() {
    ...
    window.setTimeout(() => {
        const input = contratoTareasBody.querySelector(`input[data-tarea-id="${nuevaTarea.id_tarea_cliente}"]`);
...
function renderTareasContrato() {
    if (!contratoTareasBody) return;

    contratoTareasBody.innerHTML = "";

    if (tareasContrato.length === 0) {
        if (tareasEmpty) tareasEmpty.classList.remove("hidden");
        actualizarTotalContrato();
        return;
    }

    if (tareasEmpty) tareasEmpty.classList.add("hidden");

    tareasContrato.forEach((tarea) => {
        const row = document.createElement("tr");
        const idCliente = tarea.id_tarea_cliente;
        const estado = String(tarea.estado || "Pendiente").toLowerCase() === "completada" ? "Completada" : "Pendiente";

        row.innerHTML = `
            <td>
                <input type="text" class="task-input" data-tarea-id="${escapeHtml(idCliente)}" data-field="descripcion" value="${escapeHtml(tarea.descripcion)}" placeholder="Descripción de la actividad" />
            </td>
            <td>
                <input type="number" class="task-input task-importe" data-tarea-id="${escapeHtml(idCliente)}" data-field="importe" value="${Number(tarea.importe || 0).toFixed(2)}" min="0" step="0.01" placeholder="0.00" />
            </td>
            <td>
                <span class="task-status ${estado === "Completada" ? "completed" : "pending"}">${estado}</span>
            </td>
            <td>
                <button type="button" class="action-btn delete-task" data-tarea-id="${escapeHtml(idCliente)}" title="Eliminar actividad">
...
function actualizarTotalContrato() {
    if (!totalContrato) return;

    const total = tareasContrato.reduce((suma, tarea) => suma + Number(tarea.importe || 0), 0);
    totalContrato.textContent = formatCurrency(total);
}
...
if (contratoTareasBody) {
    contratoTareasBody.addEventListener("input", (event) => {
        ...
        const idCliente = input.dataset.tareaId;
        const campo = input.dataset.field;

        const tarea = tareasContrato.find((item) => String(item.id_tarea_cliente) === String(idCliente));
        if (!tarea) return;

        if (campo === "descripcion") tarea.descripcion = input.value;
        if (campo === "importe") tarea.importe = Number(input.value || 0);

        actualizarTotalContrato();
    });

    contratoTareasBody.addEventListener("click", (event) => {
        const button = event.target.closest("button[data-tarea-id]");
        if (!button) return;

        const idCliente = button.dataset.tareaId;
        const indice = tareasContrato.findIndex((item) => String(item.id_tarea_cliente) === String(idCliente));
        if (indice === -1) return;

        const tarea = tareasContrato[indice];

        abrirConfirmacion({
            title: "Eliminar actividad",
            message: `¿Eliminar la actividad "${tarea.descripcion || "sin descripción"}"?`,
            acceptLabel: "Eliminar",
            onAccept: async () => {
                tareasContrato.splice(indice, 1);
                renderTareasContrato();
                setFeedback("Actividad eliminada del contrato.", "success");
            }
        });
    });
}
...
async function cerrarContrato(idObra, opcion, nuevoContrato = null, nuevasTareas = []) {
    if (opcion !== "nuevo_contrato" && opcion !== "finalizar_obra") {
        throw new Error("La opción de cierre del contrato es inválida.");
    }

    const payload = {
        id_obra: idObra,
        opcion: opcion,
        accion: "cerrar_contrato",
        nuevas_tareas: nuevasTareas
    };

    if (nuevoContrato) payload.nuevo_contrato = nuevoContrato;
    ...
    if (apiBase) {
        ...
        const data = await parseJsonResponse(response, "No se pudo cerrar el contrato.");
        if (!response.ok) throw new Error(data.error || "No se pudo cerrar el contrato.");
        return data;
    }
...
function abrirModalNuevoContrato(obra) {
    obraSeleccionada = obra;

    const modal = document.getElementById("nuevoContratoModal");
    const nombreObra = document.getElementById("nuevoContratoObraNombre");
    const infoAnterior = document.getElementById("nuevoContratoAnteriorInfo");
    const archivo = document.getElementById("nuevoContratoArchivo");
    const archivoNombre = document.getElementById("nuevoContratoArchivoNombre");

    if (!modal) {
        setFeedback("No se encontró el modal para crear el nuevo contrato.", "error");
        return;
    }
    ...
    const nuevasBody = document.getElementById("nuevoContratoNuevasTareasBody");
    if (nuevasBody) nuevasBody.innerHTML = "";

    if (archivo) archivo.value = "";
    if (archivoNombre) archivoNombre.textContent = "Ningún archivo seleccionado";

    if (nombreObra) nombreObra.textContent = obra.nombre || "Obra";

    if (infoAnterior) {
        infoAnterior.innerHTML = `
            <div>
                <span class="detail-label">Número de contrata</span>
                <strong>${escapeHtml(obra.numero_contrata || "-")}</strong>
            </div>
            <div>
            ...
            <div>
                ...
                <strong>${formatDate(obra.fecha_fin)}</strong>
            </div>
        `;
    }

    cargarPendientesNuevoContrato(obra.id_obra);
...
async function cargarPendientesNuevoContrato(idObra) {
    const body = document.getElementById("nuevoContratoPendientesBody");
    const wrap = document.getElementById("nuevoContratoPendientesWrap");
    const empty = document.getElementById("nuevoContratoPendientesEmpty");
    ...
    try {
        const respuesta = await fetchJson(`${API_URL}?id_obra=${encodeURIComponent(idObra)}&detalle=1`);
        const tareas = respuesta?.tareas || [];

        tareasPendientesNuevoContrato = tareas.filter(
            (tarea) => String(tarea.estado || "").toLowerCase() !== "completada"
        );

        if (tareasPendientesNuevoContrato.length === 0) {
        ...
        tareasPendientesNuevoContrato.forEach((tarea) => {
        ...
        setFeedback("No se pudieron cargar las actividades pendientes.", "error");
    }
}
...
function agregarNuevaTareaNuevoContrato() {
    const id = `nueva-${Date.now()}-${Math.random().toString(16).slice(2)}`;
...
function renderNuevasTareasNuevoContrato() {
    const body = document.getElementById("nuevoContratoNuevasTareasBody");
    const empty = document.getElementById("nuevoContratoNuevasTareasEmpty");

    if (!body) return;

    body.innerHTML = "";

    if (nuevasTareasNuevoContrato.length === 0) {
        empty?.classList.remove("hidden");
        actualizarTotalNuevoContrato();
        return;
    }
    ...
    nuevasTareasNuevoContrato.forEach((tarea, index) => {
        const tr = document.createElement("tr");

        tr.innerHTML = `
            <td>
                <input type="text" class="task-input" placeholder="Descripción de la actividad" value="${escapeHtml(tarea.descripcion)}" data-index="${index}" data-field="descripcion" />
            </td>
            <td>
                <input type="number" class="task-input" min="0" step="0.01" placeholder="0,00" value="${tarea.importe || ""}" data-index="${index}" data-field="importe" />
            </td>
            <td>
                <button type="button" class="btn-secondary task-delete-btn" data-index="${index}" title="Eliminar actividad">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
        `;
...
function actualizarNuevaTarea(index, campo, valor) {
    if (!nuevasTareasNuevoContrato[index]) return;

    if (campo === "importe") {
        nuevasTareasNuevoContrato[index][campo] = Number(valor) || 0;
    } else {
        nuevasTareasNuevoContrato[index][campo] = valor.trim();
    }
...
function actualizarTotalNuevoContrato() {
    const totalAnterior = tareasPendientesNuevoContrato.reduce(
        (total, tarea) => total + Number(tarea.importe || 0),
        0
    );

    const totalNuevas = nuevasTareasNuevoContrato.reduce(
        (total, tarea) => total + Number(tarea.importe || 0),
        0
    );

    const total = totalAnterior + totalNuevas;
    const elemento = document.getElementById("nuevoContratoTotal");

    if (elemento) elemento.textContent = formatCurrency(total);
}
...
async function guardarNuevoContrato() {
    ...
    formData.append("nuevas_tareas", JSON.stringify(
    ...
    try {
        ...
        const data = await parseJsonResponse(response, "No se pudo crear el nuevo contrato.");

        if (!response.ok || data.success === false) {
            throw new Error(data.error || "No se pudo crear el nuevo contrato.");
        }

        cerrarModalNuevoContrato();
        setFeedback(data.message || "El nuevo contrato fue creado correctamente.", "success");
        await cargarObras();
    } catch (error) {
        console.error("Error creando nuevo contrato:", error);
        setFeedback(error.message || "No se pudo crear el nuevo contrato.", "error");
    } finally {
        if (btnGuardar) {
            btnGuardar.disabled = false;
            btnGuardar.textContent = "Guardar";
...
document.addEventListener("DOMContentLoaded", () => {
    const btnAgregar = document.getElementById("btnAgregarNuevaTareaContrato");
    const body = document.getElementById("nuevoContratoNuevasTareasBody");
    const archivo = document.getElementById("nuevoContratoArchivo");
    const archivoNombre = document.getElementById("nuevoContratoArchivoNombre");
    const guardar = document.getElementById("nuevoContratoGuardar");
    const cancelar = document.getElementById("nuevoContratoCancel");
    const cerrar = document.getElementById("nuevoContratoClose");

    if (btnAgregar) {
        btnAgregar.addEventListener("click", agregarNuevaTareaNuevoContrato);
    }

    if (body) {
        body.addEventListener("input", (event) => {
            const input = event.target.closest("[data-index][data-field]");
            if (!input) return;
        ...
        body.addEventListener("click", (event) => {
            ...
            nuevasTareasNuevoContrato.splice(index, 1);
            renderNuevasTareasNuevoContrato();
        });
    }
    ...
    if (guardar) guardar.addEventListener("click", guardarNuevoContrato);
    if (cancelar) cancelar.addEventListener("click", cerrarModalNuevoContrato);
    if (cerrar) cerrar.addEventListener("click", cerrarModalNuevoContrato);
});
...
function obtenerMensajeSinResultados(busqueda, estado) {
    if (!busqueda && estado === "all") return "No hay obras registradas todavía.";
    if (busqueda && estado !== "all") return "No se encontraron obras para esa búsqueda y estado.";
    if (busqueda) return "No se encontraron obras para esa búsqueda.";
    if (estado === "active") return "No hay obras activas para mostrar.";
    if (estado === "inactive") return "No hay obras inactivas para mostrar.";
